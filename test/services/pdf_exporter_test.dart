...
void main() {
  ...
  group('careSummary — cover identity row', () {
    test('a date of birth changes the cover (byte-length delta)', () async {
      ...
      final Uint8List a = await exporter().careSummary(patient: withDob);
      final Uint8List b = await exporter().careSummary(patient: withoutDob);
      expect(isPdf(a), isTrue);
      expect(isPdf(b), isTrue);
      expect(a.length, isNot(b.length),
          reason: 'the Age row becomes a Date of birth row');
    });
  });
}
