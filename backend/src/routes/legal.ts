...
<h2>5. Subprocessors and where AI runs</h2>
<p>We use a small number of trusted service providers ("subprocessors") that act only on our instructions and only to provide the service to you. Our infrastructure provider is <strong>Cloudflare, Inc.</strong>, which hosts our backend, database, file storage, <em>and</em> the AI that powers the in-app coach and document scanning (via Cloudflare Workers AI). When you use an AI feature, the relevant parts of your care record — or a document you scan — are processed by an AI model running on Cloudflare's network to generate the reply or extraction; this can include health information about your loved one. Because the AI runs on the same infrastructure that already stores your synced care data, <strong>your data is not sent to a separate AI vendor</strong>. It is not used to train any model, and prompts and responses are not retained after the request. If we add or change a subprocessor that handles your care data, we will update this list and the effective date below.</p>
